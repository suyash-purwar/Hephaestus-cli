...
export class ResponseHandler {
  ...
  private static async execAboutCommand(): Promise<void> {
    ...
    console.log(
      gradient.fruit(
        `Hephaestus is an AI-powered CLI tool which allows you to easily find answers to any questions ranging from that stubborn error to the nature of this universe.\nIt interanally uses OpenAI's text-davinci-003 and code-davinci-002 model for generating responses.`
      )
    );
    if (!config) {
      console.log(
  ...
  private static execVersionCommand(): void {
    console.log(Stylize.flatInfo('Version: 0.2.1'));
  }
